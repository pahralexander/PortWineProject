			...
			for sprRev in [2,3,4,5]
				for pU in [true]
					for oDP in [0.25]
						...
						for approx in ["PrAp", "PuAp", "PuPrAp", "PuSAp", "BAp", "SAp", "TAp"]
								...
								numAgesLarge = scalingFactor == 3 ? 15 : 25
								agesLarge = [i for i in 1:numAgesLarge]
								...
								#define harvest yield probabilities for price definition (truncating a discrete normal distribution between 0 and 8)
								if pU
									lambda = exp((2*muH-1)/(2*(sigmaH^2))); qNorm = exp(-1/(sigmaH^2))
									pmfDivisor=sum((lambda^j)*(qNorm^(j*(j-1)/2)) for j in -200:200)
									println("pmfDivisor: ", pmfDivisor)
								...
								if approx in ["PrAp", "MDP"]
									...
									for t in 1:stateMultipliers[1]
										...
										decayProbs[t] = spzeros(stateMultipliers[1])
										decayContr = spzeros(stateMultipliers[1])
										for tTrans in 1:stateMultipliers[1]
											transState = transStates[tTrans]
											if isTransState(input,transState)
												decayProbs[t][tTrans] = foldl(*,collect(combinedDecay[input[i]][i+1,input[i]-transState[i]+1] for i in 1:numAges-1 if input[i] > 0))
												decayContr[tTrans] = (input-transState)' * supermarketContribution[2:end]
											end
										end
										dSupermarket[t] = (decayProbs[t])' * decayContr
										# tempTrans = getDecays(Int8(1),input,numAges,combinedDecay,stateMultipliers,supermarketContribution)
										# nTrans = size(tempTrans,1)
										# dStates = view(tempTrans,:,1)
										# # tProbs[transInputs[t]] = tempTrans[:,numAges+1]
										# dSupermarket[t] = (view(tempTrans,:,2))' * view(tempTrans,:,3)
										#
										# @inbounds @simd for s in 1:nTrans
										# 	decayProbs[t][Int(dStates[s])] += tempTrans[s,2]
										# end
								...
								sizehint!(stateBlendingActions,stateSpaceSize); sizehint!(stateBlendingInfo,stateSpaceSize);
								for s in Int32(1):Int32(stateSpaceSize)
									stateBlendingActions[s] = Array{Int16,1}(); statePurchasingInfo[s] = Dict{Int16,Float64}(); stateBlendingInfo[s] = Dict{Int16,Tuple{Float64,Int32}}()
										...
										if BAp || TAp
											...
											for d in demandPatterns
												if dAlternatives[d] != []
													...
													finalAlternatives = copy(actionAlternatives)
													# if length(finalAlternatives) > 1
													# 	print([blendingActionSpace[i] for i in finalAlternatives])

					...
					for sprRev in [2,3,4,5]
						for oDP in [0.25]
							for pS in pSrange
								...
								agesBase = [i for i in 1:5]
								numAgesBase = agesBase[end]

								scalingFactor = sF

								ages = [i for i in 1:numAgesBase*scalingFactor]
								numAges = ages[end]

								#initialize Wine classes
								nProducts = nP
								wineClasses = [i for i in 1:nProducts]
								targetAgesBase = nProducts > 2 ? [2,3,4] : [2,4]
								targetAges = [Int(mean([(targetAgesBase[c]-1)*scalingFactor + 1, targetAgesBase[c] * scalingFactor])) for c in wineClasses]
								detDemand = nP == 2 ? [4 for i in 1:nProducts] : [2 for i in 1:nProducts]
								...
								optBInvScale = [optimalBlendingInv[1+(i-1)*scalingFactor] for i in 1:numAgesBase]
								optPInvScale = [optimalPurchaseInv[1+(i-1)*scalingFactor] for i in 1:numAgesBase]
								...
								rule2D = Dict(p=>[Int(round(rule2DBase[p][1]*(sum(optimalPurchaseInv)/sum(optPInvBase)),digits=0)),Int(round(rule2DBase[p][2]*(sum(optimalPurchaseInv[1:sF*3])/sum(optPInvBase[1:3])),digits=0))] for p in 0:nPriceLevels-1)
								...
								optPFeature = Dict("pInv" => sum(optPInvBase[2:end])/4, "pInv2" => optPInvBase[2], "pInv3" => optPInvBase[3], "pInv4" => optPInvBase[4], "pInv5" => optPInvBase[5], "pInv23" => sum(optPInvBase[2:3])/2, "pInv34" => sum(optPInvBase[3:4])/2, "pInv45" => sum(optPInvBase[4:5])/2, "pInv234" => sum(optPInvBase[2:4])/3, "pInv345" => sum(optPInvBase[3:5])/3)
								optBFeature = Dict("bInv" => sum(optBInvBase)/5, "bInv1" => optBInvBase[1], "bInv2" => optBInvBase[2], "bInv3" => optBInvBase[3], "bInv4" => optBInvBase[4], "bInv5" => optBInvBase[5], "bInv12" => sum(optBInvBase[1:2])/2, "bInv23" => sum(optBInvBase[2:3])/2, "bInv34" => sum(optBInvBase[3:4])/2, "bInv45" => sum(optBInvBase[4:5])/2, "bInv123" => sum(optBInvBase[1:3])/3, "bInv234" => sum(optBInvBase[2:4])/3, "bInv345" => sum(optBInvBase[3:5])/3,  "bInv1234" => sum(optBInvBase[1:4])/4, "bInv2345" => sum(optBInvBase[2:5])/4)
									...
									while tuningRun <= numTuningRuns
										...
										for seedRun in 1:1
											...
											for iteration in 1:1100
												profit = 0
												#apply blending
												appliedBlending, uF = predict_blending(blendingState,blendingFFTree,bFeatureList,blendingDetTree,blendingActionSpace,blendingSpaceSize,demandFulfillment,blendingProduct,blendingClassification,demandPatterns,targetAges,detDemand,numAges,nInventoryLevels,wineClasses,optimalPurchasingInv,optimalBlendingInv,TAp,prodSpecActions,optBInvBase,optBInvScale,scalingFactor,actionContribution, possibleMixesBase, currTD, currSW, absLast, optBFeature)
										...
										if mean(seedProfits) > mean(bestProfits)
											...
											println("improvement achieved: new best profit: ", mean(bestProfits), " new best config ", currConfig)
										else
								...
								blendingState = [round(optimalBlendingInv[i]) for i in 1:numAges]
...
#------------------------------------------------#
function predict_blending(startingState,blendingFFTree,bFeatureList,blendingDetTree,blendingActionSpace,blendingSpaceSize,demandFulfillment,blendingProduct,blendingClassification,demandPatterns,targetAges,detDemand,numAges,nInventoryLevels,wineClasses,optimalPurchasingInv,optimalBlendingInv,TAp,prodSpecActions,optBInvBase,optBInvScale,sF,actionContribution, possibleMixesBase,currTD,currSW,absLast,optBFeature)
	...
	for i in 1:numAgesBase
		baseInventory[i] = mean(startingState[1+sF*(i-1):sF*i])
	end
	if absLast
		baseInventory[numAgesBase] = sum(startingState[1+sF*(numAgesBase-1):sF*numAgesBase])
	end
	...
	features["bAge"] = ((startingState' * ages)/max(1,sum(startingState))-optBAge)/optBAge
	features["bInv12"] = sum(startingState[1:2*sF]-optimalBlendingInv[1:2*sF])/(2*sF)
	features["bInv123"] = sum(startingState[1:3*sF]-optimalBlendingInv[1:3*sF])/(3*sF)
	features["bInv1234"] = sum(startingState[1:4*sF]-optimalBlendingInv[1:4*sF])/(4*sF)
	features["bInv23"] = sum(startingState[sF+1:3*sF]-optimalBlendingInv[sF+1:3*sF])/(2*sF)
	features["bInv234"] = sum(startingState[sF+1:4*sF]-optimalBlendingInv[sF+1:4*sF])/(3*sF)
	features["bInv2345"] = sum(startingState[sF+1:5*sF]-optimalBlendingInv[sF+1:5*sF])/(4*sF)
	features["bInv34"] = sum(startingState[2*sF+1:4*sF]-optimalBlendingInv[2*sF+1:4*sF])/(2*sF)
	features["bInv345"] = sum(startingState[2*sF+1:5*sF]-optimalBlendingInv[2*sF+1:5*sF])/(3*sF)
	features["bInv45"] = sum(startingState[3*sF+1:5*sF]-optimalBlendingInv[3*sF+1:5*sF])/(2*sF)
	for i in 1:numAgesBase
		features[string("bInv",i)] = sum(startingState[(i-1)*sF+1:sF*i]-optimalBlendingInv[(i-1)*sF+1:sF*i])/(sF)
	end
	if absLast
		features[string("bInv",numAgesBase)] = sum(startingState[(numAgesBase-1)*sF+1:sF*numAgesBase])-sum(optimalBlendingInv[(numAgesBase-1)*sF+1:sF*numAgesBase])
	end
	for c in wineClasses
...
function predict_purchasing(startingState,purchasingTree,pFeatureList,blendingActionSpace,blendingSpaceSize,demandFulfillment,demandPatterns,targetAges,detDemand,numAges,nInventoryLevels,wineClasses,harvestCosts,TAp,pricePurchasing,orderUpTo,orderUpToYoung,purchasingStockPrice,purchasing3StockPrice,optimalPurchaseInv,optPInvBase,optPInvScale,actionContribution,sF,rule2D,useOrderUpTo,pU,optPFeature)
	...
	invState = startingState[sF+1:end]
	avgDemandAge = sum(detDemand[c] * targetAges[c] for c in wineClasses)/sum(detDemand[c] for c in wineClasses)
	...
	ages = [i for i in sF+1:numAges]
	optPAge = ((optimalPurchaseInv[sF+1:end])' * ages)/sum(optimalPurchaseInv[sF+1:end])

	if TAp
		invLevel = sum(startingState[2:end])
		invLevelNormalized = invLevel * sum(optPInvBase)/sum(optPInvScale)
		invLevelYoung = sum(startingState[2:3*sF])
	...
	features["pInv2"] = sum(startingState[2:sF*2]-optimalPurchaseInv[2:sF*2])/(2*sF-1)
	for i in 3:numAgesBase
		features[string("pInv",i)] = sum(startingState[sF*(i-1)+1:sF*i]-optimalPurchaseInv[sF*(i-1)+1:sF*i])/(sF)
	end
	features["pInv23"] = sum(startingState[2:sF*3]-optimalPurchaseInv[2:sF*3])/(3*sF-1)
	features["pInv234"] = sum(startingState[2:sF*4]-optimalPurchaseInv[2:sF*4])/(4*sF-1)
	features["pInv34"] = sum(startingState[2*sF+1:sF*4]-optimalPurchaseInv[2*sF+1:sF*4])/(2*sF)
	features["pInv345"] = sum(startingState[2*sF+1:sF*5]-optimalPurchaseInv[2*sF+1:sF*5])/(3*sF)
	features["pInv45"] = sum(startingState[3*sF+1:sF*5]-optimalPurchaseInv[3*sF+1:sF*5])/(2*sF)
	for c in wineClasses

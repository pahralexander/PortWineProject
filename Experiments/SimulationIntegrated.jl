				...
				for sprRev in [2,3,4,5]
					for oDP in [0.25]
						for pS in pSrange
							...
							largeSpread = scalingFactor == 3 ? 8 : 14
							...
							println("nInventoryLevels: ",nInventoryLevels)
							if pU
								nPriceLevels = 9
							else
								nPriceLevels = 1
							...
							orderUpTo3 = Dict(p => zeros(3*nInventoryLevels + 1) for p in 0:nPriceLevels-1)
							...
							afterPurchasingInventory3 = zeros(3*(nInventoryLevels-1)+1); for s in 1:purchasingStateSpaceSize afterPurchasingInventory3[sum(stateSpace[afterPurchasingState[s]][1:3])+1] += stateVisitsPurchasing[s] end
							...
							purchasingStock3Price = Dict(p=>Dict(l=>[] for l in 0:(nInventoryLevels-1)*2) for p in 0:nPriceLevels-1)

							for s in 1:purchasingStateSpaceSize
								ps = purchasingStateSpace[s]
								orderUpTo[ps[1]][max(1,inventoryLevelPurchasing[s]+optimalPurchasing[s])] += stateVisitsPurchasing[s]
								orderUpTo3[ps[1]][max(1,sum(ps[2:3])+optimalPurchasing[s])] += stateVisitsPurchasing[s]
								if inventoryLevelPurchasing[s] in stockLevelsInvestigated
									...
									append!(purchasingStock3Price[ps[1]][sum(ps[2:3])], optimalPurchasing[s]*stateVisitsPurchasing[s])
								end
							end
							...
							timesAgeSold = zeros(numAges); timesAgesSoldProduct = Dict(w => zeros(numAges) for w in wineClasses); 
							for b in 1:blendingSpaceSize for i in ages timesAgeSold[i] += blendingFrequency[b]*blendingSpace[b][i]; for w in wineClasses timesAgesSoldProduct[w][i] += blendingFrequency[b]*prodSpecActions[b][w][i] end end end
